...
public class LongestAP {
    private int solveRec(int index, int diff, int[] nums) {
        if(index < 0){
            return 0;
        }
        ...
        for(int j=index-1;j>=0;j--){
            if(nums[index] - nums[j] == diff){
                ans = Math.max(ans, 1 + solveRec(j, diff,nums));
            }
        }
    ...
    public int longestArithSeqLengthRec(int[] nums) {
        ...
        for(int i=0;i< nums.length;i++){
            for(int j=i+1;j< nums.length;j++){
                ...
                ans = Math.max(ans, 2 + solveRec(i,nums[j] - nums[i],nums));
            }
        }
